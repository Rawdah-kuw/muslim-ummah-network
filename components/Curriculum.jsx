...
import { GraduationCap, PlayCircle, Play, Youtube, ArrowUpLeft, ArrowUpRight, ArrowLeft, ShieldCheck, CheckCircle2 } from "lucide-react";
  ...
  return (
    <section id="curriculum" className="py-16 md:py-24 bg-white scroll-mt-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* Science filters (full page only) */}
        {!preview && (
        ...
        {preview ? (
          <>
            {/* Overview of every science so visitors see the full scope before entering */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sciences.map(({ cat: c, items }) => {
                const lvls = [...new Set(items.map((p) => p.level))].sort();
                return (
                  <Link key={c.key} href={`/${lang}/path/${c.key}`}
                    className="bg-pearl-50 rounded-2xl border border-pearl-200 p-5 hover:shadow-pine hover:border-sage-300 transition-all flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="font-bold text-pine-800">{c[lang]}</h3>
                      <span className="text-xs text-sage-600 shrink-0 inline-flex items-center gap-1">
                        <PlayCircle size={13} /> {items.length} {t.pathCountWord}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">
                      {t.pathLevelsLabel} {lvls.map((l) => PL_LEVELS[l][lang]).join(" · ")}
                    </p>
                  </Link>
                );
              })}
            ...
            <div className="mt-8 text-center">
              <Link href={`/${lang}/curriculum`}
                ...
                {t.viewAllPath} <ArrowLeft size={16} className="rtl:rotate-180" />
              </Link>
            </div>
          </>
        ) : (
